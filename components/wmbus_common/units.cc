...
#define X(cname, lcname, hrname, quantity, explanation)                        \
  const SIUnit SI_##cname(Unit::cname);
LIST_OF_UNITS
#undef X
...
bool overrideConversion(Unit from, Unit to) {
  // The mbus protocol lacks kvarh and kva. Some meters, like the abbb23 uses
  // kwh for kvarh. Permit 1 to 1 conversion from kwh to kvarh and kva in
  // extractNumeric.
...
bool isKCF(const SIExp &e) {
  return e == SI_K.exp() || e == SI_C.exp() || e == SI_F.exp();
}

bool is_S_MONTH_YEAR_UT(const SIExp &e) {
  return e == SI_Second.exp() || e == SI_UnixTimestamp.exp() ||
         e == SI_Month.exp() || e == SI_Year.exp();
}
...
bool SIUnit::convertTo(double left, const SIUnit &out_siunit,
                       double *out) const {
...
bool forbidden_op(MathOp op, const SIExp &a, const SIExp &b) {
  // Two unix timestamps cannot be added together. They can be subtracted
  // though!
  if (op == MathOp::ADD && a == SI_UnixTimestamp.exp() &&
      b == SI_UnixTimestamp.exp())
    return true;
...
bool SIUnit::mathOpTo(MathOp op, double left, double right,
  ...
  if (exp() == SI_UnixTimestamp.exp() ||
      right_siunit.exp() == SI_UnixTimestamp.exp()) {
    ...
    assert(exp() == SI_UnixTimestamp.exp() &&
           right_siunit.exp() != SI_UnixTimestamp.exp());

    // The timestamp is left. Lets handle all permitted additions to
    // UnixTimestamp.
...
#define X(cname, si_scale, si_exponents)                                       \
  if ((scale_ == si_scale) && (exponents_ == (si_exponents)) &&                \
      quantity_ == toQuantity(Unit::cname))                                    \
    return Unit::cname;
  LIST_OF_SI_CONVERSIONS
#undef X
...
bool SIExp::operator==(const SIExp &e) const {
  return s_ == e.s_ && m_ == e.m_ && kg_ == e.kg_ && a_ == e.a_ &&
         mol_ == e.mol_ && cd_ == e.cd_ && k_ == e.k_ && c_ == e.c_ &&
         f_ == e.f_ && month_ == e.month_ && year_ == e.year_ &&
